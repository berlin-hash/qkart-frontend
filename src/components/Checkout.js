  ...
  return (
    <Box display="flex" flexDirection="column">
      ...
      <Stack direction="row" my="1rem">
        ...
        <Button variant="text" onClick={handleNewAddress}>
          Cancel
        </Button>
      </Stack>
    </Box>
...
const Checkout = () => {
  ...
  const validateRequest = (items, addresses) => {
    ...
    const orderValue = getTotalCartValue(items);
    console.log(addresses);
  ...
  const performCheckout = async (token, items, addresses) => {
    if (validateRequest(items, addresses)) {
      try {
        ...
        const balance = localStorage.getItem("balance");
    const orderValue = getTotalCartValue(items);
          localStorage.setItem("balance", balance - orderValue);
                ...
                addresses.all.map((addressObj) => (
                  <Box
                    ...
                    className={`address-item ${
                      addressObj.address === addresses.address
                        ? "selected"
                        : "not-selected"
                    }`}
                  >
                    <p onClick={() => handleSelectAddress(addressObj._id)}>
                      {addressObj.address}
                    </p>

                    <Button
                      startIcon={<Delete />}
                      onClick={() => deleteAddress(token, addressObj._id)}
                    >
                      DELETE
                    </Button>
                  </Box>
